...
docker_tests(){ step "Docker-Simulation ausführen"; if ! command_exists docker; then warn "Docker fehlt; Docker-Simulation wird übersprungen."; return 0; fi; docker build -f "$REPO_ROOT/tests/docker/Dockerfile.ubuntu-test" -t shopware-infra-template-ubuntu-test "$REPO_ROOT"; docker run --rm --privileged -e SHOPWARE_INFRA_TEST_MODE=1 shopware-infra-template-ubuntu-test bash tests/docker/run-staging-test.sh; docker run --rm --privileged -e SHOPWARE_INFRA_TEST_MODE=1 shopware-infra-template-ubuntu-test bash tests/docker/run-production-test.sh; }
...
case "$MODE" in syntax) SHOPWARE_INFRA_TOTAL=2; syntax_tests ;; bats) SHOPWARE_INFRA_TOTAL=1; bats_tests ;; docker) SHOPWARE_INFRA_TOTAL=1; docker_tests ;; testinfra) SHOPWARE_INFRA_TOTAL=1; testinfra_tests ;; all) SHOPWARE_INFRA_TOTAL=3; syntax_tests; bats_tests; docker_tests ;; *) die "Unbekannter Testmodus: $MODE" ;; esac
